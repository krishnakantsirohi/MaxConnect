...
public class GameBoard
{
     ...
     * @param player the player whose score is being requested.  valid
     * values are 1 or 2
     * @return the number of 111's or 222's in the current board.
     */
    private int triple1(int player){
        int playerScore = 0;
        ...
        for( int i = ROW_LENGTH-1; i >= 0; i-- )
        {
            for( int j = 0; j < 4; j++ )
            {
                ...
                {
                    if (i < ROW_LENGTH-1 && this.playBoard[i+1][j+3]!=0)
                    playerScore++;
                }
                if( (     this.playBoard[ i ][j] == 0 ) &&
                        ( this.playBoard[ i ][ j+1 ] == player ) &&
                        ...
                        ( this.playBoard[ i ][ j+3 ] ==  player) )
                {
                    if (i < ROW_LENGTH-1 && this.playBoard[i+1][j] != 0)
                    playerScore++;
                }
            }
        } // end horizontal
        ...
        for( int i = ROW_LENGTH-1; i >= 3; i-- ){
            for( int j = 3; j < COLUMN_LENGTH; j++ ) {
                if( ( this.playBoard[ i ][ j ] == player ) &&
                    ...
                    if (i < ROW_LENGTH-1 && playBoard[i-2][j-3]!=0)
                    playerScore++;
                }
                if( ( this.playBoard[ i ][ j ] == 0 ) &&
                        ( this.playBoard[ i-1 ][ j-1 ] == player ) &&
                        ( this.playBoard[ i-2 ][ j-2 ] == player ) &&
                        ( this.playBoard[ i-3 ][ j-3 ] == player ) ) {
                    if (i < ROW_LENGTH-1 && playBoard[ i+1 ][ j ]!=0)
                    playerScore++;
                }
            }
        }
        ...
        for( int i = ROW_LENGTH-1; i >= 3; i-- ){
            for( int j = 0; j < 4; j++ ) {
                if( ( this.playBoard[ i ][ j ] == player ) &&
                        ...
                        ( this.playBoard[ i-2 ][ j+2 ] == player ) &&
                        ( this.playBoard[ i-3 ][ j+3 ] == 0 ) ) {
                    if (i<ROW_LENGTH-1 && playBoard[i+1][j+3]!=0)
                    playerScore++;
                }
                if( ( this.playBoard[ i ][ j ] == 0 ) &&
                        ( this.playBoard[ i-1 ][ j+1 ] == player ) &&
     ...
     * @param player the player whose score is being requested.  valid
     * values are 1 or 2
     * @return the number of 11's or 22's in the current board.
     */
    private int double1(int player){
        int playerScore = 0;
     ...
     * @param player the player whose score is being requested. Valid
     * values are 1 or 2
     * @return the total utility value of the current board.
     */
    public int utility(int player)
    {
        return ((getScore(player)*4)+(triple1(player)*3)+(double1(player)*2));
    }
    ...
    public void printGameBoard()
    {
        ...
        for( int i = 0; i < 6; i++ )
        {
            ...
            for( int j = 0; j < 7; j++ )
            {
                System.out.print( this.playBoard[i][j]==0?"  ":playBoard[i][j] + " " );
            }
